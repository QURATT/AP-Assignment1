...
public class Queue{
	public int addPointer; //points to the position where new number is to be added.
	public int removePointer; //points tot position of the element, which is to be deleted. 
	int sizeofQueue;   //number of elements in the queue.
	public int [] array;      //array to store values.

	public Queue(int size){
		addPointer=0;
		removePointer=0;
		sizeofQueue=0;
		setArray(new int[size+1]); //because addPointer and removePointer cannot point at same index,so in order to create a buffer i.e space of one box or index between them.
	}
	...
	public void enqueue(int number) throws Exception{
		...
		if(addPointer==getArray().length-1){
			if(addPointer+1-getArray().length==removePointer){
				throw new Exception("Cannot enqueue, Queue is full"); 
			}
			getArray()[addPointer]=number;
			sizeofQueue++;
			addPointer=0;
		}
		else{
	...
	public int dequeue() throws Exception{
		...
		if(removePointer==getArray().length-1){
			removePointer=0;
		}
		sizeofQueue--;
		removePointer++;
		return getArray()[removePointer-1];
	}
